...
from .views import convert_scheduler_log_update_body, call_scheduler_run_log
...
class RepeatTask(Task):
    ...
    def before_start(self, task_id, args, kwargs):
        ...
        try:
            ...
            if periodic_task is None:
                ...
                return
            push_job_data_to_scheduler(job_id=task_detail.job_id, next_run_date=next_run_date,
                                       state='RUNNING')
        except Exception as e:
    ...
    def after_return(self, status, retval, task_id, args, kwargs, einfo):
        ...
        try:
            ...
            operation = 'RUN'
            if job_settings.max_failure == job_settings.failure_count:
                operation = 'BROKEN'
#            elif job_settings.max_run == task_detail.run_count:
#                operation = 'COMPLETED'
            request_body = convert_scheduler_log_update_body(task_detail, job_settings, retval, operation)
            if request_body is not None:
                call_scheduler_run_log(request_body, 'update')
        except Exception as e:
            logger.error(f'Error in after_return for {task_id}: {e}')


def push_job_data_to_scheduler(job_id, next_run_date, state):
    url = f'{BASE_URL}/job/updateData'
    ...
    try:
        ...
        logger.info(f'Success push job data to scheduler with body {body}')
        return True
    except requests.RequestException as e:
        logger.error('Error in pushing job data to scheduler: %s', e)
        return False
